...
class View(object):
    ...
    def caricaInterfaccia(self):
        ...
        self._datePicker = ft.DatePicker(
            first_date=datetime.datetime(2002, 11, 1),
        ...
        self._btnCalendar = ft.ElevatedButton(
            ...
            on_click=lambda e: self._page.open(self._datePicker),
        )

        row1 = ft.Row(
            [self._txtIn, self._txtCFU, self._ddVoto, self._btnCalendar],
            alignment=ft.MainAxisAlignment.CENTER,
        )

        # Row 2
        self._btnAdd = ft.ElevatedButton("Add", on_click=self._controller.handleAdd)
        self._btnPrint = ft.ElevatedButton(
            "Print", on_click=self._controller.handlePrint
        )

        row2 = ft.Row(
            [self._btnAdd, self._btnPrint], alignment=ft.MainAxisAlignment.CENTER
        )
